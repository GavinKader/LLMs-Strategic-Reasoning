...
log_likelihood_abs <- function(params, x) {
  ...
  sf <- 0.0001 # scale factor for MLE convergence
...
for(llm in LLMs){
  print(llm)
  filename_data <- paste0("Data files/bcg_1shot_avgmed_",llm, ".csv")
  ...
  output_avgmed <- matrix(NA,length(unique_treatments),29)
  colnames(output_avgmed)<-c("LLM", "p0","p1","p2","p3","p4","p5","var1", "var2", "var3", "var4", "var5", "n", "treatment","temperature","ci_lower_p0","ci_upper_p0","ci_lower_p1","ci_upper_p1","ci_lower_p2","ci_upper_p2","ci_lower_p3","ci_upper_p3","ci_lower_p4","ci_upper_p4","ci_lower_p5","ci_upper_p5","ci_lower_n","ci_upper_n")
  sf <- 0.0001
...
result <- maxLik(logLik = log_likelihood_abs, start = initial_params, x = beauty_data$y, method = "BFGS")
optimized_params <- result$estimate
params <- exp(optimized_params[1:5]) / (1 + sum(exp(optimized_params[1:5])))
n <- exp(sf*optimized_params[6])+round(50*(2/3)^1)
...
output_avgmed[i,2:13]<-results
output_avgmed[i,14]<-treatment
output_avgmed[i,15]<-unique(beauty_data$temp)
...
bootstrap_estimates <- pblapply(1:n_bootstrap, function(b) {
  ...
  tryCatch({
    bootstrap_result <- maxLik(logLik = log_likelihood_abs, start = initial_params, x = bootstrap_sample$y, method = "BFGS")
...
output_avgmed[i, "ci_lower_p1"] <- ci_lower_p1
output_avgmed[i, "ci_upper_p1"] <- ci_upper_p1
output_avgmed[i, "ci_lower_p2"] <- ci_lower_p2
output_avgmed[i, "ci_upper_p2"] <- ci_upper_p2
output_avgmed[i, "ci_lower_p3"] <- ci_lower_p3
output_avgmed[i, "ci_upper_p3"] <- ci_upper_p3
output_avgmed[i, "ci_lower_p4"] <- ci_lower_p4
output_avgmed[i, "ci_upper_p4"] <- ci_upper_p4
output_avgmed[i, "ci_lower_p5"] <- ci_lower_p5
output_avgmed[i, "ci_upper_p5"] <- ci_upper_p5
output_avgmed[i, "ci_lower_p0"] <- ci_lower_p0
output_avgmed[i, "ci_upper_p0"] <- ci_upper_p0
...
#filename_graph <- paste0("", llm, "/avgmed/",treatment, ".pdf") Set directory of your own choosing
ggsave(filename_graph, width = 8, height = 4, units = "in", device = "pdf")
}
  
#filename_table <- paste0("", llm, "/avgmed/output_avgmed.csv") Set directory of your own choosing
...
#combined_filename <- "output_avgmed_allLLMs.csv" Set directory of your own choosing
