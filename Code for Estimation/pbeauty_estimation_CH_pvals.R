...
for (treatment in unique_treatments) {
  ...
  if (unique(beauty_data$p_val)=="twothirds"){
    ...
    output_pvals[i,10]<-unique(beauty_data$temp)
    n_bootstrap <- 1000
...
#filename_table <- paste0("", llm, "/pvals/2+ people/output_pvals_CH.csv") Change to own directory
...
#combined_filename <- "output_pvals_CH_allLLMs.csv" Change to own directory
