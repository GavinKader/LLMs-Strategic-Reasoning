...
LLMs<-c("gemini2","chatgpto1","claude3")
...
log_likelihood_gg <- function(params) {
  #responses
  {
    order <- c(6,15,14,10,9,2,12,3,16,11,4,13,8,1,7,5)
...
for(llm in LLMs){
  i=-1
  print(llm)
  filename_data <- paste0("Data files/gg_nofeedback_",llm, ".csv")
  ...
  for (treatment in unique_treatments) {
    ...
    output[i+j,26]<-unique(GG_data$temp)
    sd_list[[j]]<-sd
    }
  ...
  #filename_table <- paste0("/", llm, "/output_lk.csv")  choose own directory
  write.csv(output, file = filename_table, row.names = TRUE)
  #filename_table <- paste0(", llm, "/output_sd_lk.csv")  choose own directory
...
#combined_filename <- paste0() choose own directory
