...
for(llm in LLMs){
  ...
  for (treatment in unique_treatments) {
    ...
    for (j in 1:nrow(GG_data_full)){
      ...
      output[k+j,21]<-unique(GG_data$temp)
      sd_list[[j]]<-sd
  ...
  # filename_table <- paste0("", llm, "/output_ch.csv") choose own directory
  write.csv(output, file = filename_table, row.names = TRUE)
  # filename_table <- paste0("", llm, "/output_sd_ch.csv")choose own directory
