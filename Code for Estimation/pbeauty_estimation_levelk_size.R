...
for (llm in LLMs) {
  ...
  output_size <- matrix(NA,length(unique_treatments),29)
  colnames(output_size)<-c("LLM", "p0","p1","p2","p3","p4","p5","var1", "var2", "var3", "var4", "var5", "n", "treatment","temperature","ci_lower_p0","ci_upper_p0","ci_lower_p1","ci_upper_p1","ci_lower_p2","ci_upper_p2","ci_lower_p3","ci_upper_p3","ci_lower_p4","ci_upper_p4","ci_lower_p5","ci_upper_p5","ci_lower_n","ci_upper_n")
  sf <- 0.0001
...
for (treatment in unique_treatments) {
  ...
  n <- exp(optimized_params[6])+round(50*(2/3)^1)
  ...
  output_size[i,1]<-llm
  output_size[i,2:13]<-results
  output_size[i,14]<-treatment
  output_size[i,15]<-unique(beauty_data$temp)
  ...
  output_size[i, "ci_lower_p1"] <- ci_lower_p1
  output_size[i, "ci_upper_p1"] <- ci_upper_p1
  output_size[i, "ci_lower_p2"] <- ci_lower_p2
  output_size[i, "ci_upper_p2"] <- ci_upper_p2
  output_size[i, "ci_lower_p3"] <- ci_lower_p3
  output_size[i, "ci_upper_p3"] <- ci_upper_p3
  output_size[i, "ci_lower_p4"] <- ci_lower_p4
  output_size[i, "ci_upper_p4"] <- ci_upper_p4
  output_size[i, "ci_lower_p5"] <- ci_lower_p5
  output_size[i, "ci_upper_p5"] <- ci_upper_p5
  output_size[i, "ci_lower_p0"] <- ci_lower_p0
  output_size[i, "ci_upper_p0"] <- ci_upper_p0
  ...
  #filename_graph <- paste0("", llm, "/size/",treatment, ".pdf") choose own directory
  ggsave(filename_graph, width = 8, height = 4, units = "in", device = "pdf")
}
  #filename_table <- paste0("", llm, "/size/output_size.csv") choose own directory
